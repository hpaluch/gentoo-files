...
src_install() {
	...
	/bin/sh -c "${BUNDLE} exec rake yarn:install RAILS_ENV=${RAILS_ENV} NODE_ENV=${NODE_ENV}" || die "yarn install failed"
	/bin/sh -c "${BUNDLE} exec rake gitlab:assets:clean RAILS_ENV=${RAILS_ENV} NODE_ENV=${NODE_ENV}" || die "assets clean failed"
	/bin/sh -c "${BUNDLE} exec rake gitlab:assets:compile RAILS_ENV=${RAILS_ENV} NODE_ENV=${NODE_ENV}" || die "assets compile failed"
